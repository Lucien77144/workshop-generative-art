...
export class GrassGeometry extends THREE.BufferGeometry {
    constructor(params) {
        ...
        const getRdmCoordFromAxe = (axe) => {
            return (params.size[axe] / 2) * (Math.random() - 0.5) * 2
        }
        ...
        this.setAttribute(
            ...
            new THREE.BufferAttribute(
                new Float32Array(new Float32Array(positions)),
                3
            )
        )
    ...
    computeBlade(center, index = 0) {
        ...
        const bl = bladeData[0].map(
            (n, i) => n * (BLADE_WIDTH / 2) * 1 + center[i]
        ) // bottom left
        const br = bladeData[0].map(
            (n, i) => n * (BLADE_WIDTH / 2) * -1 + center[i]
        ) // bottom right
        const tl = bladeData[0].map(
            (n, i) => n * (BLADE_WIDTH / 4) * 1 + center[i]
        ) // top left
        const tr = bladeData[0].map(
            (n, i) => n * (BLADE_WIDTH / 4) * -1 + center[i]
        ) // top right
